...
- Prompt (MANDATORY, NO VARIATION):
  ...
  > (All workflows indexed at `~/.back2base/meta/workflows/index.md`)"
...
- Check for `~/.back2base/meta/workflows/index.md`.
...
- For chosen workflow:
- Load from:
  - Config: `~/.back2base/meta/workflows/{workflow_name}/config.md`
  - Role prompts: `~/.back2base/meta/workflows/{workflow_name}/roles/{agent}.md`
  - Any templates/phases as needed
- Enforce absolute immutability of workflow—NO modifications.
- Initialize output directory:
  ```
  ~/.back2base/outputs/{workflow_name}_{timestamp}/
  ```
- For each phase N:
  - Prepare per-phase directory:
    ```
    ~/.back2base/outputs/{workflow_name}_{timestamp}/phase{N}/
    ```
  - For each agent in phase:
    - Create task file:
      ```
      ~/.back2base/outputs/{workflow_name}_{timestamp}/phase{N}/{agent}_task.md
      ```
    - Agent receives ONLY:
      - Persona prompt: `~/.back2base/meta/workflows/{workflow_name}/roles/{agent}.md`
      - Context injected via:  
        ```
        <task_path>~/.back2base/outputs/{workflow_name}_{timestamp}/phase{N}/{agent}_task.md</task_path>
        ```
    - Agent outputs to:
      ```
      ~/.back2base/outputs/{workflow_name}_{timestamp}/phase{N}/{agent}_output.md
      ```
  - If critic present for phase:
    ...
    - Critic outputs to:
      ```
      ~/.back2base/outputs/{workflow_name}_{timestamp}/evaluations/phase{N}_{agent}_eval.md
      ```
    - **MANDATORY LOOP:**  
      ...
      - All loop artifacts (every iteration) are logged as:
        ```
        ~/.back2base/outputs/{workflow_name}_{timestamp}/phase{N}/history/{agent}_output_iter{I}.md
        ~/.back2base/outputs/{workflow_name}_{timestamp}/evaluations/history/phase{N}_{agent}_eval_iter{I}.md
        ```
  - After final phase, copy or symlink final outputs to:
    ```
    ~/.back2base/outputs/{workflow_name}_{timestamp}/final/
    ```
    and optionally
    ```
    ~/.back2base/meta/workflows/{workflow_name}/roles/final_{timestamp}/
    ```
- At completion, display:
  ```
  ✅ Workflow {workflow_name} executed successfully.
  Final outputs: ~/.back2base/outputs/{workflow_name}_{timestamp}/final/
  ```
- TERMINATE.
...
3. **Augmentations:** If `~/.back2base/newskills.md` exists, display it and prompt: "Specify any new skills/tools to enable."
...
- For each agent:
  - Persona prompt at:
    ```
    ~/.back2base/meta/workflows/{workflow_name}/roles/{agent}.md
    ```
    - Must include:  
...
- For each critic:
  - Persona prompt at:
    ```
    ~/.back2base/meta/workflows/{workflow_name}/roles/{critic}.md
    ```
...
- Save to:
- Config: `~/.back2base/meta/workflows/{workflow_name}/config.md`
- Roles: `~/.back2base/meta/workflows/{workflow_name}/roles/{agent}.md`
- Templates, phases, as needed.
- Append workflow to:
````

~/.back2base/meta/workflows/index.md
...
\- Config: ~/.back2base/meta/workflows/{workflow\_name}/config.md
\- Roles: ~/.back2base/meta/workflows/{workflow\_name}/roles/
\- Outputs: ~/.back2base/outputs/{workflow\_name}\_{timestamp}/
...
- Every round, all outputs, tasks, and evaluations are to be saved under
  ```
  ~/.back2base/outputs/{workflow_name}_{timestamp}/phase{N}/
  ~/.back2base/outputs/{workflow_name}_{timestamp}/evaluations/
  ```
